...
class DhakaJi {
    ...
    private int dfs(ArrayList<Integer> []adjList,int n){
        ...
        // out.println(Arrays.toString(in));
        // out.println(first);
        // out.println(max);             
        max=dfs2(adjList,first,0);
        // dfs2(adjList,1,0);
        // int max=-1;
        // for(int i=1;i<=n;i++){
        //     max=Math.max(max,Math.max(in[i],ou[i]));
        // }        
        // out.println(Arrays.toString(ou));
        return max ;
    }
    private int dfs2(ArrayList<Integer> []adjList,int vertex,int parent){
        int max=0;
        for(int child : adjList[vertex]){
            if(child==parent)
                continue ;
            int x=dfs2(adjList,child,vertex)+1;
            if(x>max)
                max=x;
        }
        return max ;
    ...
    private void dfs1(ArrayList<Integer> []adjList,int vertex,int parent){
        ...
        for(int child : adjList[vertex]){
            if(child==parent)
                continue ;
            in[child]=1+in[vertex];
            dfs1(adjList,child,vertex);
            // if(x>max)
            //     max=x;
            // in[vertex]=Math.max(in[vertex],in[child]+1);
        }
    }    
